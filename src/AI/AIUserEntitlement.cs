...
namespace Sufficit.AI
{
    ...
    /// Verification: use HasPolicy(principal, new AIUserEntitlement()) to check possession
    /// on any context (typical case), or HasPolicy&lt;AIUserEntitlement&gt;(principal, contextId)
    /// to scope the check to a specific customer context.
    /// This entitlement governs consumption only. For module administration (providers, presets,
    /// configuration), see AIControlEntitlement.
    /// </remarks>
    public class AIUserEntitlement : Entitlement, ISelfContextEntitlement
    {
        ...
        // No role association — entitlement possession alone grants access
